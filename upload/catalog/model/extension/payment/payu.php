...
class ModelExtensionPaymentPayu extends Model
{
	public function getMethod($address, $total)
	{
		...
		if ($this->config->get('payment_payu_total') > $total) {
			$status = false;
		} else if (!$this->config->get('payment_payu_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				'code'       => 'payu',
				'title'      => !empty($this->config->get('payment_payu_title')) ? $this->config->get('payment_payu_title') : $this->language->get('text_title'),
				'terms'      => '',
